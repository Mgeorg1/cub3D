...
{
	if (all->map[(int)(all->plr.y  - all->plr.pln.x * MOVE_SPD)][(int)all->plr.x] != '1')
		all->plr.y -= all->plr.pln.x * MOVE_SPD;
	if (all->map[(int)(all->plr.y)][(int)(all->plr.x + all->plr.pln.y * MOVE_SPD)] != '1')
		all->plr.x += all->plr.pln.y * MOVE_SPD;
}

void	key_a(t_all *all)
{
	if (all->map[(int)(all->plr.y  - all->plr.pln.y * MOVE_SPD)][(int)all->plr.x] != '1')
		all->plr.y -= all->plr.pln.y * MOVE_SPD;
	if (all->map[(int)(all->plr.y)][(int)(all->plr.x - all->plr.pln.x * MOVE_SPD)] != '1')
